...
static cl::opt<bool>
   only_globals("only_globals", cl::init(false), cl::Hidden,
...
static cl::opt<bool>
    no_exit("no_exit", cl::init(false), cl::Hidden,
  ...
  struct Func_call : public ModulePass {
    ...
    std::vector<Value*> load_call_names(std::string caller, std::string callee, Instruction* instr_after, LLVMContext &module_context){
        ...
        Instruction *callee_assigned_array = GetElementPtrInst::CreateInBounds(callee_assigned_type,
                                                                                ...
                                                                                ConstantInt::get(Type::getInt8Ty(module_context), 0),
                                                                                "callee_name",
                                                                                instr_after  );  //load @callee_name string  <- Type: [N x i8] (array)
    ...
    void parse_value(Value* call_rec, Value* &value, Type* &val_type, Instruction *insert_point, unsigned mode, Module &M, LLVMContext& module_context){
        ...
        if (val_type->isPointerTy()){
        ...
        }else if (val_type->isStructTy() //the rest conditions are optional just for the circumstances
                  && !dyn_cast<StructType>(val_type)->isLiteral()
                  && (val_type->getStructName() == "class.std::__cxx11::basic_string")){
                      // || val_type->getStructName() == "struct.std::__cxx11::basic_string<char, std::char_traits<char>, std::allocator<char> >::_Alloc_hider"
                      // || val_type->getStructName() == "union.anon")){


            std::string type_str;
            type_str = val_type->getStructName(); //literal structs never have names
            ...
            int member_size = val_type->getStructNumElements();

            if (val_type->getStructName() == "class.std::__cxx11::basic_string"){
              member_size--;
            }

            CallInst *update_val_struct = CallInst::Create(update_struct, {type_cast, ConstantInt::get(Type::getInt64Ty(module_context), member_size), call_rec, ConstantInt::get(Type::getInt32Ty(module_context), mode)}, "", insert_point); (void) update_val_struct;
            //TODO. val_type->getStructName() must be appended to globals and then loaded and bitcasted. Is it worth the effort ??

            // for (unsigned i = 0; i < member_size; i++){ //TODO change this member_size for generic implementation

                Instruction *struct_elem = ExtractValueInst::Create(value, 0, "struct_access", insert_point);
                Instruction *struct_elem_inside = ExtractValueInst::Create(struct_elem, 0, "struct_access", insert_point);
                Value *struct_val = struct_elem_inside;
                Type *struct_elem_type = struct_elem_inside->getType();

                parse_value(call_rec, struct_val, struct_elem_type, insert_point, mode, M, module_context);

                Instruction *struct_elem_second = ExtractValueInst::Create(value, 1, "struct_access", insert_point);
                Value *struct_val_second = struct_elem_second;
                Type *struct_elem_type_second = val_type->getStructElementType(1);

                parse_value(call_rec, struct_val_second, struct_elem_type_second, insert_point, mode, M, module_context);
            ...
            if (val_type->isStructTy()){
                ...
                if (!dyn_cast<StructType>(val_type)->isLiteral()){
                    type_str = val_type->getStructName(); //literal structs never have names
                    /////////////////////////////////////////
                    /////debug block for multiprecision nums////
                    ////////////////////////////////////////////
                    // if (type_str == "class.boost::multiprecision::number"){
                      outs() << "\n\n";
                      val_type->print(outs());
                      outs() << "\n\n";
    ...
    void trace_global_variables(Module &M){
        ...
        // global_trace = M.getOrInsertFunction("global_tracing_function", Type::getVoidTy(module_context));
        // Function *global_tracing_func = cast<Function>(global_trace);
    ...
    void declare_extern_funcs(Module &M, LLVMContext &module_context){
        ...
        // trace_function = M.getOrInsertFunction("_Z14trace_functionPKc", Type::getVoidTy(module_context), Type::getInt8PtrTy(module_context));
    ...
    bool setup_main_exit(Module &M, LLVMContext& module_context){
      ...
      for (SymbolTableList<BasicBlock>::iterator BB = main->begin(); BB != main->end(); BB++){
        ...
        return false; //no call to global_trace and print_trace instrumented
      }
      return true;
    }
    ...
    void update_unwind_block(BasicBlock *unw, BasicBlock *source, Value *cur_index, LLVMContext& module_context){
    	...
    	if (accessed_unwind_blocks.find(unw) == accessed_unwind_blocks.end()){ // not found. Make a new one
	...
	// cl::opt<std::string> OutputFilename("asdas", cl::desc("Specify output filename"), cl::value_desc("filename"));

		// outs() << "Arg given: " << My_arg << "\n";
        ...
        if (only_globals && trace_globals){
          outs() << "Tracing only globals...\n";
