...
namespace {
  struct Func_call : public ModulePass {
    ...
    std::unordered_map<std::string, GlobalVariable*> struct_types; //maps struct types with the global variables allocated with their name
    std::unordered_set<Type*> structs_parsed; // used to keep track of recursive relations when parsing structdecls
        ...
        if (!dyn_cast<StructType>(t)->isLiteral()){
          ...
          if (
            ...
            t->getStructName().find("class.seal::BigUInt") != std::string::npos  ||
              ...
              t->getStructName().find("class.seal::SmallModulus") != std::string::npos  ||
              t->getStructName().find("class.seal::SEALContext") != std::string::npos  ||
              // t->getStructName().find("class.seal::KeyGenerator") != std::string::npos  ||  //prob group 1
              t->getStructName().find("class.seal::Ciphertext") != std::string::npos  ||
              t->getStructName().find("class.seal::PublicKey") != std::string::npos  ||
              t->getStructName().find("class.seal::SecretKey") != std::string::npos  ||
              t->getStructName().find("class.seal::Plaintext") != std::string::npos  ||
              t->getStructName().find("class.seal::Encryptor") != std::string::npos  ||
              // t->getStructName().find("class.seal::Decryptor") != std::string::npos  || //prob group 2
              t->getStructName().find("class.seal::IntegerEncoder") != std::string::npos  ||
              t->getStructName().find("class.seal::util::Pointer") != std::string::npos ){
            */
