...
void deallocate_memory(){
	...
	// std::cout << "DEBUG MESSAGE: func_call size: " << func_calls.size() << " global_variables size: " << global_variables.size() << std::endl;
	return;
}

void print_trace(){

	std::cout << "----------------------\nTrace Function" << std::endl;
	int real_func_counter = 0;
	for (std::vector<function_call*>::iterator it = func_calls.begin(); it != func_calls.end(); it++){

		(*it)->print();
		real_func_counter++;
	}
	// std::cout << "----------------------\nGlobal variables Trace" << std::endl;
	// int real_global_counter = 0;
	...
	// }
	// // std::cout << "----------------------\n" << func_calls.size() << " function calls / " << global_variables.size() << " global variables\n";
	// std::cout << "----------------------\n" << real_func_counter << " function calls / " << real_global_counter << " global variables\n";
	...
	if(stop_trace != 0){
		switch(type){
			...
			case 2:
				...
				break;
			default:
				assert("Wrong update type given");
				break;
		}
	}
